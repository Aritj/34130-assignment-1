...
def split_step(A_in, z, w, beta2, beta3, alpha, gamma, N_seg):
    ...
    for _ in range(N_seg):
        ...
        A_w *= np.exp((first_term - alpha/2) * dz)
...
def plot_separate_per_C():
    for C in C_values:
        ...
        for j, z in enumerate(z_values):
            ...
            # Time domain subplot
            axs[0, j].plot(t * 1e12, np.abs(A_in)**2 / np.max(np.abs(A_in)**2), 'b-', label=f'Input')
            axs[0, j].plot(t * 1e12, np.abs(A_out)**2 / np.max(np.abs(A_in)**2), 'r--', label=f'Output')
            ...
            axs[1, j].plot(f * 1e-12, np.abs(A_in_w)**2 / np.max(np.abs(A_in_w)**2), 'b-', label='Input')
            axs[1, j].plot(f * 1e-12, np.abs(A_out_w)**2 / np.max(np.abs(A_in_w)**2), 'r--', label='Output')
            axs[1, j].set_xlim([-1, 1])
            axs[1, j].set_xlabel('Frequency (THz)', fontsize=10)
            axs[1, j].set_ylabel('Normalized Power Spectrum', fontsize=10)
            axs[1, j].set_title(f'Frequency Domain (C={C}, z = {z:.4f} km)', fontsize=11)
            axs[1, j].grid()
